...
@Component({
  selector: 'app-cheatsheet-ts',
  templateUrl: './cheatsheet-ts.html',
  styleUrl: './cheatsheet-ts.scss',
})
export class CheatsheetTs {
  sections: Section[] = [
    {
...
function makeNoise(pet: Cat | Dog) {
  if ('meow' in pet) pet.meow();
        ...
        {
          label: 'Discriminated union & type predicate',
          code: `type Shape =
  | { kind: 'circle'; radius: number }
...
const s: Status = Status.Active;  // 'ACTIVE'`,
        ...
        {
          label: 'Const enums & as const alternative',
          code: `// const enum — inlined at compile time
...
class Product {
  ...
  toString() {
    return \`area=\${this.area()}, p=\${this.perimeter()}\`;
  }
}
  ...
  async delete(id: number)   {}
    ...
    },
    {
      ...
      items: [
        {
          label: 'as, satisfies, !',
          code: `// as — override inferred type (no runtime effect)
