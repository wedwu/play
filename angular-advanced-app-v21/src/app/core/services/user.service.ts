...
export class UserService extends EntityStateService<User> {
   ...
   */
  readonly byDepartment$: Observable<Record<Department, User[]>> =
    this.items$.pipe(
      map((users) => {
        ...
        return depts.reduce(
          (acc, d) => ({
            ...acc,
            [d]: users.filter((u) => u.department === d),
          }),
  ...
  getUsersForProject = (memberIds: string[]): Observable<User[]> =>
    this.items$.pipe(
      map((users) => users.filter((u) => memberIds.includes(u.id))),
    );
  ...
  protected override applyFilterAndSort = (
    ...
    return sort.field
      ? sortBy(
          result,
          (u) => String(u[sort.field as keyof User] ?? ""),
          sort.direction,
        )
  ...
  private seedUsers = (): void => {
    ...
    admin.addSkill("TypeScript");
    admin.addSkill("Angular");
    admin.addSkill("Leadership");
    ...
    this.addMany([
      admin,
      makeUser("Bob", "Martin", "bob@acme.com", "manager", "engineering", [
        "Node.js",
        "PostgreSQL",
      ]),
      makeUser("Carol", "White", "carol@acme.com", "developer", "engineering", [
        "React",
        "CSS",
      ]),
      makeUser("Dave", "Kim", "dave@acme.com", "developer", "design", [
        "Figma",
        "CSS",
      ]),
      makeUser("Eva", "Patel", "eva@acme.com", "developer", "product", [
        "Python",
        "Analytics",
      ]),
    ]);
