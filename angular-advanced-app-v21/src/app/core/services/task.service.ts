...
export class TaskService extends EntityStateService<Task> implements OnDestroy {
   ...
   */
  readonly tasksByStatus$: Observable<Record<TaskStatus, Task[]>> =
    this.items$.pipe(
      map((tasks) =>
        Object.values(TaskStatus).reduce(
          (acc, s) => ({
            ...acc,
            [s]: tasks.filter((t) => t.taskStatus === s),
          }),
  ...
  readonly totalStats$ = this.items$.pipe(
    map((tasks) => ({
      ...
      inProgress: tasks.filter((t) => t.taskStatus === TaskStatus.IN_PROGRESS)
        .length,
   ...
   * `STATUS_TRANSITIONS` rules. Shows a success notification when the
  ...
  transitionTask = (taskId: string, newStatus: TaskStatus): void => {
    ...
    if (result.success) {
      ...
      if (newStatus === TaskStatus.DONE)
        this.notifications.success("Task completed", task.title);
    } else {
  ...
  getByAssignee = (userId: string): Observable<Task[]> =>
    this.items$.pipe(
      map((tasks) => tasks.filter((t) => t.assigneeId === userId)),
    );
  ...
  protected override applyFilterAndSort = (
    ...
    if (filter["status"])
      result = result.filter((t) => t.taskStatus === filter["status"]);
        ...
        tap(() => {
          ...
          if (overdue.length > 0)
            this.notifications.warn(
              "Overdue tasks",
              `${overdue.length} task(s) past due date`,
            );
        }),
      )
      .subscribe();
  ...
  private seedData = (): void => {
    const tasks: Task[] = [
      new TaskBuilder("Set up CI/CD pipeline", "u1")
        ...
        .withTag("devops", "#4ECDC4")
        .withTag("infrastructure", "#45B7D1")
        .withEstimate(8)
        .withDueDate(new Date(Date.now() + 86400000 * 3))
        .build(),
      ...
      new TaskBuilder("Database query optimization", "u1")
        ...
        .withTag("performance", "#FF6B6B")
        .withEstimate(5)
        .build(),
    ];

    tasks[0].taskStatus = TaskStatus.IN_PROGRESS;
    tasks[0].loggedHours = 3;
