import { BaseEntity, EntityStatus } from './base-entity.model';
...
describe('BaseEntity', () => {
  ...
  beforeEach(() => { entity = new TestEntity('tester'); });
  ...
  describe('generateId', () => {
    ...
    it('generates unique values', () => {
      const ids = new Set(Array.from({ length: 100 }, () => BaseEntity.generateId()));
      expect(ids.size).toBe(100);
    });
  });

  describe('constructor', () => {
    it('assigns a non-empty id', () => expect(entity.id.length).toBeGreaterThan(0));
    it('sets createdBy', () => expect(entity.createdBy).toBe('tester'));
    it('defaults createdBy to "system"', () => expect(new TestEntity().createdBy).toBe('system'));
    it('sets createdAt to approximately now', () => {
      expect(Date.now() - entity.createdAt.getTime()).toBeLessThan(500);
    });
    it('initialises status to "active"', () => expect(entity.status).toBe('active'));
  });
  ...
  describe('archive', () => {
    it('sets status to "archived"', () => { entity.archive(); expect(entity.status).toBe('archived'); });
    it('calls touch',               () => { const spy = jest.spyOn(entity, 'touch'); entity.archive(); expect(spy).toHaveBeenCalled(); });
  });
  ...
  describe('softDelete', () => {
    it('sets status to "deleted"', () => { entity.softDelete(); expect(entity.status).toBe('deleted'); });
  });

  describe('isActive', () => {
    it('returns true when active',   () => expect(entity.isActive()).toBe(true));
    it('returns false when archived', () => { entity.archive(); expect(entity.isActive()).toBe(false); });
  });
