...
describe('Task', () => {
  ...
  beforeEach(() => { task = new Task('Fix login bug', 'Auth is broken', TaskPriority.HIGH, 'dev'); });

  // ── Constructor ───────────────────────────────

  describe('constructor', () => {
    it('sets title and description',  () => { expect(task.title).toBe('Fix login bug'); expect(task.description).toBe('Auth is broken'); });
    it('sets priority',               () => expect(task.priority).toBe(TaskPriority.HIGH));
    it('defaults status to TODO',     () => expect(task.taskStatus).toBe(TaskStatus.TODO));
    it('defaults loggedHours to 0',   () => expect(task.loggedHours).toBe(0));
    it('defaults description to ""',  () => expect(new Task('T').description).toBe(''));
    it('defaults priority to MEDIUM', () => expect(new Task('T').priority).toBe(TaskPriority.MEDIUM));
  });

  // ── Validation ────────────────────────────────

  describe('validate', () => {
    it('passes for title 3–200 chars', () => expect(new Task('abc').validate()).toBe(true));
    it('fails for title < 3 chars',    () => expect(new Task('ab').validate()).toBe(false));
    it('fails for empty title',        () => expect(new Task('').validate()).toBe(false));
    it('fails for title > 200 chars',  () => expect(new Task('x'.repeat(201)).validate()).toBe(false));
  });
  ...
  describe('progress', () => {
    it('is 0 for a TODO task with no subtasks', () => expect(task.progress).toBe(0));
  ...
  describe('daysUntilDue', () => {
    it('returns null when no due date', () => expect(task.daysUntilDue).toBeNull());
  ...
  describe('addTag', () => {
    it('adds a tag', () => { task.addTag('bug', '#f00'); expect(task.tags).toHaveLength(1); });
