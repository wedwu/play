...
describe('Project', () => {
  ...
  beforeEach(() => { project = new Project('Nexus', 'Core rebuild', 'u1', 'product'); });
  ...
  describe('constructor', () => {
    ...
    it('defaults phase to "planning"', () => expect(project.phase).toBe('planning'));
    it('assigns a color string',       () => expect(project.color.startsWith('#')).toBe(true));
    it('assigns an emoji icon',        () => expect(project.iconEmoji.length).toBeGreaterThan(0));
  });

  // ── Validation ────────────────────────────────

  describe('validate', () => {
    it('passes for a valid project',      () => expect(project.validate()).toBe(true));
    it('fails for name shorter than 2',   () => { project.name = 'X'; expect(project.validate()).toBe(false); });
    it('fails for missing ownerId',       () => { project.ownerId = ''; expect(project.validate()).toBe(false); });
  });

  // ── Members ───────────────────────────────────

  describe('addMember / removeMember', () => {
    it('adds a member',         () => { project.addMember('u2'); expect(project.memberIds).toContain('u2'); });
    it('ignores duplicates',    () => { project.addMember('u2'); project.addMember('u2'); expect(project.memberIds).toHaveLength(1); });
    it('removes a member',      () => { project.addMember('u2'); project.removeMember('u2'); expect(project.memberIds).not.toContain('u2'); });
  });
  ...
  describe('health', () => {
    it('is "green" when completion ≥ 80%', () => {
      ...
      for (let i = 0; i < 2; i++) project.addTask(new Task(`P${i}`, '', TaskPriority.LOW));
  ...
  describe('addMilestone / completeMilestone', () => {
    it('creates and returns a milestone', () => {
      const m = project.addMilestone('Alpha', new Date(Date.now() + 86_400_000));
  ...
  describe('activeSprint', () => {
    it('returns undefined when no active sprint', () => expect(project.activeSprint).toBeUndefined());
