...
describe("sortBy", () => {
  ...
  it("sorts descending", () => {
    expect(sortBy(items, (i) => i.n, "desc").map((i) => i.n)).toEqual([
      3, 2, 1,
    ]);
  });
...
describe("capitalize", () => {
  it("uppercases first letter and lowercases rest", () =>
...
describe("Validators", () => {
  it("required: false for null", () =>
    expect(Validators.required(null)).toBe(false));
  it("required: false for empty string", () =>
    expect(Validators.required("")).toBe(false));
  it("required: true for non-empty string", () =>
    expect(Validators.required("x")).toBe(true));
  it("email: valid address passes", () =>
    expect(Validators.email("a@b.com")).toBe(true));
  it("email: invalid address fails", () =>
    expect(Validators.email("not-an-email")).toBe(false));
  it("minLen: passes at boundary", () =>
    expect(Validators.minLen(3)("abc")).toBe(true));
  it("minLen: fails below boundary", () =>
    expect(Validators.minLen(3)("ab")).toBe(false));
  it("range: passes within range", () =>
    expect(Validators.range(1, 10)(5)).toBe(true));
  it("range: fails outside range", () =>
    expect(Validators.range(1, 10)(11)).toBe(false));
