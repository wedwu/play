...
describe('authGuard', () => {
  ...
  it('returns false when auth_token is absent', () => {
    expect(TestBed.runInInjectionContext(() => authGuard(route, state))).toBe(false);
  });
...
describe('roleGuard', () => {
  ...
  const run = (...roles: UserRole[]): Observable<boolean> =>
    TestBed.runInInjectionContext(() => roleGuard(...roles)(route, state)) as Observable<boolean>;

  it('emits true for a user with the required role', done => {
    selected$.next(makeUser('admin'));
    run('admin').subscribe(result => { expect(result).toBe(true); done(); });
  });
  ...
  it('emits false when no user is selected', done => {
    selected$.next(null);
    run('admin').subscribe(result => { expect(result).toBe(false); done(); });
  });

  it('accepts multiple allowed roles', done => {
    selected$.next(makeUser('manager'));
    run('admin', 'manager').subscribe(result => { expect(result).toBe(true); done(); });
  });
});
