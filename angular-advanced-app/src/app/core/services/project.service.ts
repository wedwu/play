...
export class ProjectService extends EntityStateService<Project> {
   ...
   * In ecosystems like RxJS (commonly used in Angular), developers append $ to a variable name to signal:
   * “This variable is an Observable (a stream of values over time)”. This convention helps developers quickly identify which variables are Observables and which are not, improving code readability and maintainability.
   */
  ...
  protected override applyFilterAndSort = (
    ...
    if (filter["phase"])
      result = result.filter(
        (p) => p.phase === (filter["phase"] as ProjectPhase),
      );
    return sort.field
      ? sortBy(
          result,
          (p) => String(p[sort.field as keyof Project] ?? ""),
          sort.direction,
        )
  ...
  private seedProjects = (): void => {
    const p1 = new Project(
      "Nexus Platform",
      "Core product rebuild with new architecture",
      "u1",
      "product",
      "system",
    );
    ...
    const p2 = new Project(
      "DevOps Revamp",
      "Modernize CI/CD and infrastructure",
      "u2",
      "infrastructure",
      "system",
    );
